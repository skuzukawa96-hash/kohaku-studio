...
- メモリ: 起動時 約10MB / 30万行CSVロード後 約30MB / 200万行（87MB CSV）ロード後 約130MB。
  ただしインポート処理中は一時的にCSVサイズの約10倍のピーク（200万行で約900MB）が発生する
- インポート: 30万行 約1.3秒 / 200万行 約12秒（Parquetキャッシュからの再読込は約5秒）
- SQL集計: 200万行のGROUP BYで1〜3.5秒 / 分析（プロファイル・エルボー法など）: 1〜4秒
