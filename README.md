...
2. **SQLタブ** — 取り込んだデータセットはすべてSQLのテーブルとして参照可能。
   ...
   エディタは構文に色が付く（キーワード / 文字列 / 数値、`--` と `/* */` のコメントはグレー）
3. **チャートタブ** — 棒 / 折れ線 / 散布図 / ヒストグラム / テーブル / ウェハーマップ / SPC管理図。
   データセットまたはSQL結果をソースに、X列・Y列・集計（合計/平均/件数/最小/最大）を指定。
   棒・折れ線・散布図・ヒストグラムは「系列（色分け）」でカテゴリ別に色分けできる
   （ヒストグラムは半透明の重ね描きで分布を比較。ビンの区切りは全系列で共通）。
   さらに「分割: 列（横）」で列を選ぶと、その値ごとに小さなグラフを
   横に並べて描く（例: 装置別に散布図を5枚並べる。Y軸・ビン・系列の色は
   パネル間で共通なので、そのまま見比べられる。値が多い場合は先頭12件）。
   「分割: 行（縦）」に2つ目の列を足すと、**行×列の格子**（seaborn の FacetGrid 相当）で
   並ぶ（例: 地域×製品で売上推移を一覧。列見出しを上、行見出しを左に置き、
   同じ列は縦に・同じ行は横に揃うので2つのカテゴリを同時に見比べられる。各軸 先頭6件まで）。
   ウェハーマップはダイ座標（X/Y）と値の列を指定すると色分け格子＋カラースケールで表示。
   「分割」にウェハーID列を指定すると複数枚を一括表示できる（最大25枚=1ロット分。
   カラースケールと座標範囲は全ウェハー共通なので、不良パターンの違いをそのまま見比べられる。
   `--make-samples` の `sample_wafer.csv`（6枚分）で試せる）。
   SPC管理図は順序列と測定値を指定すると CL/UCL/LCL（移動範囲法の±3σ）と
   ネルソンルール（3σ超え / 9点連続同側 / 6点連続増減）の違反点を赤で表示。
   「分割」に装置IDなどを指定すると**装置ごとに管理限界を引き直して一括作図**でき、
   どの装置が異常かを一覧で判断できる（Y軸レンジはパネル間で共通なので水準差も見比べられる。
   データ数が足りない装置はそのパネルにだけ理由が出る）。
   ...
   **X軸の範囲**も同様に指定できる（散布図・折れ線・ヒストグラムなど、X軸が数値のグラフで有効）。
...
7. **テーマ切り替え** — ヘッダーの太陽 / 月アイコンのボタンでダークとライトを切り替え。
...
データプロファイル・回帰分析・クラスタリング・時系列分解では「**グループ分割**」に列を選ぶと、
その値ごと（装置別・ロット別など）に分けて分析を実行し、結果を縦に並べて比較できます
（チャートのファセット表示の分析版）。SPC管理図はチャート側の「分割」で同じことができます。

- 一度に実行するのは先頭12グループまで（超えた場合は「N件中 先頭12件を表示」と明示）
- **1つのグループが失敗しても他は表示される**（例: 1台だけデータ数不足でも、残りの装置の結果は見られる）
- 空欄にすれば従来どおりデータ全体で1回だけ実行
- クラスタリングのグループ分割では、散布図は先頭2特徴量で描かれ、軸の選択と
  結果のデータセット保存は使えません（グループごとに別の結果になるため）
...
- 分析（プロファイル・回帰・クラスタリング・時系列分解・装置差分析・SPC）が読むのは
  先頭100万行まで。超えた場合は**結果の先頭に警告を表示**するので、部分データと
  気づかずに解釈することはない
- チャートの表示上限: 棒グラフは先頭60カテゴリ、SPC管理図は10,000点、
  ファセットは1次元12枚（ウェハーマップのみ25枚）/ 2次元は各軸6件。
  いずれも超えた分は「N件中 先頭M件を表示」と画面に明示される
