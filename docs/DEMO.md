...
8. **装置差分析**（分析タブ → 装置差分析） — **まず分析タブ上部の「データソース」を
   `measurements` に切り替える**（既定は最初に取り込んだ `sample_sales` のままで、
   そのままだと `tool_id` / `yield` が選択肢に出ない）。
   グループ列 `tool_id`、測定値 `yield`
   → Welch ANOVA が装置間の有意差を検出し、有意差のあるペアに含まれる装置に
   「差あり」が付く（測定値の良し悪しの向きはデータから決まらないため、
   どれが「悪い」かはツールは判定しない）

9. **ロットトレース**（分析タブ → ロットトレース） — ID列 `lot_id`、`LOT0042` で検索
   （ロットトレースはデータソースの選択に関係なく、全データセットを横断する）
   → `lot_id` 列を持つ全データセットから該当行が横断的に集まる

10. **回帰分析**（分析タブ → 回帰分析） — データソースは `measurements` のまま。
    目的変数 `yield`、説明変数 `defects`
    → 高い決定係数（R² ≈ 0.99）とフィット直線付き散布図が表示される

11. **クラスタリング**（分析タブ → クラスタリング） — **データソースを `sample_sales` に戻す**
    （`units` / `unit_price` はこちらの列）。特徴量 `units` と `unit_price`。
...
ヘッダーの太陽 / 月アイコンのボタンで切り替えられ、選択はブラウザに保存されます。
