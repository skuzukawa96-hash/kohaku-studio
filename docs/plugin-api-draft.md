...
- **v1.x 実装で得た知見(集計の絞り込み)**: `form.agg` は `true`(全集計を選ばせる)のほかに
  配列を取れる。`agg: ["avg", "sum"]` のように書くと、その集計だけがセレクトに出る。
  チャートが実装していない集計を選べると、選んだ内容と描かれるものが食い違うため
  (SPC管理図は avg/sum しか実装していないのに件数・最小・最大も選べていた)。
  **配列の先頭が既定値**になるので、既定にしたい集計を先に書く
...
- **本体は `render` を呼ぶ前に ctx の文字状態(`textAlign` / `textBaseline`)を既定へ戻す**。
  ファセットの見出しを描いた状態が残ったまま呼ぶと、基準線を自分で設定していない
  チャートの文字が下へずれて枠外に出る(v0.7 で実際に発生: SPC管理図の情報行が
  下段の見出しと重なり、最下段は見切れた)。チャート側も、複数種類の文字を描くなら
  そのつど `textAlign` / `textBaseline` を明示するのが安全
