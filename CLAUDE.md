...
- ロードマップは `ROADMAP.md`（現在 v0.5 完了 = Plugin API Phase 1 + アイコン/テーマ + チャート軸の修正 + 分析APIのメモリ削減）。v0.6 以降の候補は ROADMAP に記載（Plugin API Phase 2/3、時系列予測、データプロファイルのメモリ削減）。次の計画はオーナーと相談して決める。
