...
- ロードマップは `ROADMAP.md`（現在 v0.6 完了 = カラム値別の可視化 = ヒストグラムの系列色分け + ファセット表示 + ウェハーマップのファセット対応）。v0.7 以降の候補は ROADMAP に記載（Plugin API Phase 2/3、分析のグループ別実行、時系列予測、データプロファイルのメモリ削減）。次の計画はオーナーと相談して決める。
