...
- ロードマップは `ROADMAP.md`（現在 v0.7 完了 = 分析のグループ別実行 = `/api/analyze/group` + 回帰/時系列分解の「グループ分割」 + SPC管理図のファセット表示）。v0.8 以降の候補は ROADMAP に記載（Plugin API Phase 2/3、時系列予測、データプロファイルのメモリ削減、グループ別実行の対応分析の追加）。次の計画はオーナーと相談して決める。
