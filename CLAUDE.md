...
- ロードマップは `ROADMAP.md`（現在 v0.8 完了 = データプロファイルの列単位読み込みによる省メモリ化 + グループ別実行にプロファイル/クラスタリングを追加）。v0.9 以降の候補は ROADMAP に記載（Plugin API Phase 2/3、時系列予測）。次の計画はオーナーと相談して決める。
